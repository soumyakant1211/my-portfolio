...
const Experience = () => {
  return (
    <section id="experience" className="py-10 md:py-12 scroll-mt-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-violet-300 dark:before:via-violet-900/50 before:to-transparent">
        {experiences.map((exp) => (
          <div key={exp.id} className="relative flex items-start group">
            <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-slate-50 dark:border-slate-950 bg-violet-100 dark:bg-violet-900/40 text-violet-500 shrink-0 shadow-sm z-10 group-hover:scale-110 group-hover:bg-violet-500 group-hover:text-white transition-all mt-6">
              <Briefcase size={16} />
            </div>
            
            <div className="w-[calc(100%-4rem)] ml-6 p-6 md:p-8 rounded-3xl glass transition-all hover:-translate-y-1 hover:shadow-2xl hover:shadow-violet-500/10 border border-transparent hover:border-violet-500/20">
